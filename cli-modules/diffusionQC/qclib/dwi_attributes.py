...
def dwi_attributes(file_name):
    ...
    b_value = float(hdr['keyvaluepairs']['DWMRI_b-value'])
    gradients = np.zeros((mri.shape[grad_axis],3), dtype= 'float')

    for i in range(mri.shape[grad_axis]):
        # Python 3.6
        # gradients[i,: ]= [float(x) for x in hdr['keyvaluepairs']['DWMRI_gradient_'+f'{i:04}'].split( )]

        #Python 2.7
        gradients[i, :] = [float(x) for x in hdr['keyvaluepairs']['DWMRI_gradient_' + '{:04}'.format(i)].split()]
